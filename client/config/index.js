...
let version
try {
  let lastGitTagHash = ChildProcess.execSync('git rev-list --tags --max-count=1').toString().trim()
  console.log('last git tag hash => ')
  console.log(lastGitTagHash)
  let lastGitTag = ChildProcess.execSync(`git describe --tags ${lastGitTagHash}`).toString().trim()
